...
$stock_query = $mysqli->query("SELECT * FROM stock_items ORDER BY categorie, nom");
$stock = [];
if ($stock_query) {
...
$categories_query = $mysqli->query("SELECT DISTINCT categorie FROM stock_items ORDER BY categorie");
$categories = [];
if ($categories_query) {
...
<head>
  ...
  <title>St Sofie | Gestion de Stock</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Gestion de Stock
        <small>Livres, fournitures et matériel informatique</small>
      </h1>
      <ol class="breadcrumb">
      ...
      <div class="row">
        <div class="col-md-3">
          <div class="box box-solid">
            ...
            <div class="box-body no-padding">
              <ul class="nav nav-pills nav-stacked">
                <li class="active"><a href="#" data-filter="all"><i class="fa fa-inbox"></i> Tous les articles
                  <span class="label label-primary pull-right"><?php echo count($stock); ?></span></a>
                </li>
                <?php foreach ($categories as $categorie): ?>
                <li><a href="#" data-filter="<?php echo $categorie; ?>"><i class="fa fa-folder"></i> <?php echo $categorie; ?>
                  <?php 
                    $count = 0;
                  ...
                  ?>
                  <span class="label label-primary pull-right"><?php echo $count; ?></span></a>
                </li>
                <?php endforeach; ?>
              </ul>
            </div>
          ...
          <div class="box box-solid">
            ...
            <div class="box-body">
              <button type="button" class="btn btn-primary btn-block margin-bottom" data-toggle="modal" data-target="#modal-ajouter-article">
                <i class="fa fa-plus"></i> Ajouter un article
              </button>
              <button type="button" class="btn btn-warning btn-block margin-bottom" data-toggle="modal" data-target="#modal-mouvement">
                <i class="fa fa-exchange"></i> Enregistrer un mouvement
              </button>
              <a href="<?php echo BASE_URL; ?>index.php?controller=directrice&action=exportStock" class="btn btn-default btn-block">
                <i class="fa fa-download"></i> Exporter l'inventaire
              </a>
            </div>
            <!-- /.box-body -->
          </div>
        ...
        <div class="col-md-9">
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">Inventaire du stock</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table id="stock-table" class="table table-bordered table-striped">
                ...
                <tr class="item-row" data-category="<?php echo $item['categorie']; ?>">
                  <td><?php echo $item['id']; ?></td>
                  <td><?php echo $item['nom']; ?></td>
                  <td><?php echo $item['categorie']; ?></td>
                  <td>
                    <?php if ($item['quantite'] <= $item['seuil_alerte']): ?>
                      <span class="label label-danger"><?php echo $item['quantite']; ?></span>
                    <?php else: ?>
                      <?php echo $item['quantite']; ?>
                    <?php endif; ?>
                  </td>
                  <td><?php echo $item['seuil_alerte']; ?></td>
                  <td><?php echo !empty($item['date_dernier_mouvement']) ? date('d/m/Y', strtotime($item['date_dernier_mouvement'])) : 'Jamais'; ?></td>
                  <td>
                    <button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#modal-details-<?php echo $item['id']; ?>">
                      <i class="fa fa-eye"></i> Détails
                    </button>
                    <button type="button" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#modal-modifier-<?php echo $item['id']; ?>">
...
<div class="modal fade" id="modal-ajouter-article">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="<?php echo BASE_URL; ?>index.php?controller=directrice&action=ajouterArticle" method="post">
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <input type="number" class="form-control" name="seuil_alerte" id="seuil_alerte" min="0" required>
          </div>
          <div class="form-group">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title">Enregistrer un mouvement de stock</h4>
      </div>
      <form action="<?php echo BASE_URL; ?>index.php?controller=directrice&action=mouvementStock" method="post">
        <div class="modal-body">
          <div class="form-group">
            ...
            <select class="form-control" name="article_id" id="article_id" required>
              ...
              <?php foreach ($stock as $item): ?>
                <option value="<?php echo $item['id']; ?>"><?php echo $item['nom']; ?> (<?php echo $item['categorie']; ?>) - Stock actuel: <?php echo $item['quantite']; ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="form-group">
            <label for="type_mouvement">Type de mouvement</label>
            <select class="form-control" name="type_mouvement" id="type_mouvement" required>
          ...
          <div class="form-group">
            ...
            <input type="number" class="form-control" name="quantite_mouvement" id="quantite_mouvement" min="1" required>
          </div>
          <div class="form-group">
            <label for="date_mouvement">Date du mouvement</label>
            <input type="date" class="form-control" name="date_mouvement" id="date_mouvement" value="<?php echo date('Y-m-d'); ?>" required>
          </div>
          <div class="form-group">
