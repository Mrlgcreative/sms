          ...
          <div class="box box-info">
            <div class="box-header with-border">
              ...
              <div class="box-tools pull-right">
                ...
                <a href="<?php echo BASE_URL; ?>index.php?controller=Director&action=ajouterEvenement" class="btn btn-success btn-sm"><i class="fa fa-plus"></i> Ajouter un événement</a>
              </div>
            </div>
            <div class="box-body">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      <a href="<?php echo BASE_URL; ?>index.php?controller=Director&action=modifierEvenement&id=<?php echo $evenement['id']; ?>" class="btn btn-xs btn-warning"><i class="fa fa-edit"></i> Modifier</a>
                      <a href="<?php echo BASE_URL; ?>index.php?controller=Director&action=supprimerEvenement&id=<?php echo $evenement['id']; ?>" class="btn btn-xs btn-danger delete-event" data-id="<?php echo $evenement['id']; ?>"><i class="fa fa-trash"></i> Supprimer</a>
                    </td>
                  </tr>
                  <?php endforeach; ?>
                </tbody>
  ...
  $(function () {
    ...
    $('#calendar').fullCalendar({
      ...
      eventClick: function(calEvent, jsEvent, view) {
        ...
        $.ajax({
          url: '<?php echo BASE_URL; ?>index.php?controller=Director&action=getEvenementDetails',
          type: 'GET',
          data: {id: calEvent.id},
          dataType: 'json',
    ...
    $('#save-event').on('click', function() {
      $.ajax({
        url: '<?php echo BASE_URL; ?>index.php?controller=Director&action=updateEvenement',
        type: 'POST',
    ...
    $('#add-new-event').click(function (e) {
      ...
      var eventData = {
        titre: val,
        date_debut: today.toISOString().slice(0, 16).replace('T', ' '),
        date_fin: tomorrow.toISOString().slice(0, 16).replace('T', ' '),
        lieu: 'À déterminer',
        responsable: '<?php echo $username; ?>',
        description: 'Nouvel événement créé par le directeur',
        couleur: currColor
      };
      
      $.ajax({
        url: '<?php echo BASE_URL; ?>index.php?controller=Director&action=ajouterEvenementRapide',
        type: 'POST',
        data: eventData,
        dataType: 'json',
        success: function(response) {
          if(response.success) {
            ...
            alert('Événement créé avec succès! Vous pouvez le modifier pour ajouter plus de détails.');
          } else {
            alert('Erreur lors de la création de l\'événement: ' + response.message);
          }
        },
        error: function() {
