...
// Récupération des comptables depuis la table users
$comptables = [];
$query = "SELECT id, username, email,password, telephone, adresse FROM users WHERE role = 'comptable' ORDER BY username, email";
...
// Gestion de la suppression d'un comptable
if (isset($_GET['delete_id']) && !empty($_GET['delete_id'])) {
    $delete_id = intval($_GET['delete_id']);
    
    $delete_query = "DELETE FROM users WHERE id = ? AND role = 'comptable'";
    $stmt = $mysqli->prepare($delete_query);
    $stmt->bind_param("i", $delete_id);
    
    if ($stmt->execute()) {
        $_SESSION['message'] = "Comptable supprimé avec succès.";
        ...
        $_SESSION['message'] = "Erreur lors de la suppression du comptable: " . $mysqli->error;
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="treeview active">
          ...
          <ul class="treeview-menu">
            ...
            <li class="active"><a href="<?php echo BASE_URL; ?>index.php?controller=Admin&action=comptable"><i class="fa fa-circle-o"></i> Voir</a></li>
          </ul>
        </li>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Liste des comptables</h3>
              <div class="box-tools">
            ...
            <div class="box-body" id="printable">
              <table id="comptablesTable" class="table table-bordered table-striped">
                <thead>
                  <tr>
                    <th>ID</th>
                ...
                <tbody>
                  <?php foreach ($comptables as $comptable): ?>
                    <tr>
                      ...
                      <td><?php echo $comptable['username']; ?></td>
                      <td><?php echo $comptable['email']; ?></td>
                      
                      <td><?php echo $comptable['telephone']; ?></td>
                      <td><?php echo $comptable['adresse']; ?></td>
                      <td class="no-print">
                        <a href="<?php echo BASE_URL; ?>index.php?controller=Admin&action=editcomptable&id=<?php echo $comptable['id']; ?>" class="btn btn-info btn-xs">
                          <i class="fa fa-pencil"></i> Modifier
                        </a>
                        <a href="<?php echo BASE_URL; ?>index.php?controller=Admin&action=comptable&delete_id=<?php echo $comptable['id']; ?>" class="btn btn-danger btn-xs" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce comptable?');">
...
<script>
  ...
  function printContent(elementId) {
    ...
    document.body.innerHTML = '<h1 style="text-align: center;">Liste des Comptables</h1>' + content;
    window.print();
