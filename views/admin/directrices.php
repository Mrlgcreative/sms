...
// Récupération des directrices depuis la table users
$directrices = [];
$query = "SELECT id, username,  email, telephone as contact, adresse FROM users WHERE role = 'directrice' ORDER BY username, email";
...
// Gestion de la suppression d'une directrice
if (isset($_GET['delete_id']) && !empty($_GET['delete_id'])) {
    $delete_id = intval($_GET['delete_id']);
    
    $delete_query = "DELETE FROM users WHERE id = ? AND role = 'directrice'";
    $stmt = $mysqli->prepare($delete_query);
    $stmt->bind_param("i", $delete_id);
    
    if ($stmt->execute()) {
        $_SESSION['message'] = "Directrice supprimée avec succès.";
        ...
        $_SESSION['message'] = "Erreur lors de la suppression de la directrice: " . $mysqli->error;
...
<div class="wrapper">
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="treeview">
          ...
          <ul class="treeview-menu">
            ...
            <li class="active"><a href="<?php echo BASE_URL; ?>index.php?controller=Admin&action=directrices"><i class="fa fa-circle-o"></i> Voir Directrices</a></li>
          </ul>
        </li>
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <h1>
        Gestion des Directrices
        <small>Liste des directrices</small>
      </h1>
      <ol class="breadcrumb">
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Liste des directrices</h3>
              <div class="box-tools">
            ...
            <div class="box-body" id="printable">
              <table id="directricesTable" class="table table-bordered table-striped">
                <thead>
                  <tr>
                    <th>N°</th>
                    <th>Contact</th>
                ...
                <tbody>
                  <?php $index = 0; foreach ($directrices as $directrice): $index++; ?>
                    <tr>
                      <td><?php echo $index; ?></td>
                      <td><?php echo $directrice['telephone']; ?></td>
                      <td><?php echo $directrice['email']; ?></td>
                      <td><?php echo $directrice['adresse']; ?></td>
                      <td><?php echo $directrice['section']; ?></td>
                      <td class="no-print">
                        <a href="<?php echo BASE_URL; ?>index.php?controller=Admin&action=editdirectrice&id=<?php echo $directrice['id']; ?>" class="btn btn-info btn-xs">
                          <i class="fa fa-pencil"></i> Modifier
                        </a>
                        <a href="<?php echo BASE_URL; ?>index.php?controller=Admin&action=directrices&delete_id=<?php echo $directrice['id']; ?>" class="btn btn-danger btn-xs" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette directrice?');">
...
<script>
  ...
  function printContent(elementId) {
    ...
    document.body.innerHTML = '<h1 style="text-align: center;">Liste des Directrices</h1>' + content;
    window.print();
